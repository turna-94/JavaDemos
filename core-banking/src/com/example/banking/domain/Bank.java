...
public class Bank {
	...
	public long getAccountNumber(Class<? extends Account> clazz) {
		return customers.stream().map(Customer::getAccounts).flatMap(Collection::stream).filter(clazz::isInstance)
				.count();
	}
	
	public long getTotalNumberOfAccounts() {
		return customers.stream().map(Customer::getAccounts).mapToLong(Collection::size).sum();
	}
